...
<section class=" w-full  bg-bgColor py-5   min-h-screen">
    ...
    <div class="container lg:w-1/2 w-3/4 ">
        ...
        <div>
            ...
            <base-intro :CourseDes="CourseDes" :CourseName="CourseName"></base-intro>
        </div>
            ...
            <ul  class=" space-y-5" v-for="(el,key) in Chapters" :key="key">
                <li class=" my-2 cursor-pointer select-none" @click="showLessons(el,key)" ref="myli"  >
                 ...
                 <ul ref="list" class="subMenu itemRef space-y-2 my-2">
                        ...
                        <li v-for="(lesson,key) in courseLessons" :key="key" class=" relative py-6  "  :class="lesson.chapterId == el.id ?'':'hidden'">
...
let currentCoureData = this.$store.getters['courses/allCourses'].filter(el => el.id == CourseId);
        ...
        // ............. chapters
        let allChapters = this.$store.getters['courses/CoureChapters'].filter(el => el.CourseId == CourseId);
