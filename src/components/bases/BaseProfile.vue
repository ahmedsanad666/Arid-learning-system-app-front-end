<template>
  <Teleport to="body">
    ...
    <div  @click="$emit('Close')" class=" overLay absolute top-0 left-0 h-[200vh] w-full   "></div>
      <section  class="py-4 px-3  rounded-2xl  shadow-2xl  absolute-center   md:w-[60%] w-[80%] border bg-slate-300">
          <h2 class="my-2 text-center md:text-2xl text-xl">تحديث البيانات</h2>
          <base-spinner v-if="isLoading"></base-spinner>
    <form
      @submit.prevent="updateUser"
      class=" h-[60vh] rounded-2xl shadow-xl "
      >
      <div class="flex md:flex-row-reverse flex-col gap-2  justify-between px-4">
          <div class="md:w-1/3    ">
          <img
...
try{
  ...
  await  this.$store.dispatch('students/UpdateUser',payload);
    await this.$store.dispatch('students/AllUsers')
    location.reload();
...
console.log(this.$store.getters['students/allUsers']);
    ...
    this.error = e.message || "failed to udpate reload the page and try again ";
}
